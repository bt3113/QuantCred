export function detectSchema(rows) {
  ...
  const dateColumn = columns.find((column) => column.toLowerCase() === "date") || columns.find((column) => column.toLowerCase().includes("date")) || columns[0];
  const lower = new Set(columns.map((column) => column.toLowerCase()));
  if (["symbol", "side", "quantity", "price"].every((column) => lower.has(column))) return { mode: "trades", dateColumn, returnColumns: [] };
  if (["symbol", "weight"].every((column) => lower.has(column))) return { mode: "positions", dateColumn, returnColumns: [] };
  ...
  const numericColumns = columns.filter((column) => !ignored.has(column));
  if (returnColumn) return { mode: "single", dateColumn, returnColumns: [returnColumn] };
  if (numericColumns.length >= 2) return { mode: factorLike(numericColumns) ? "factor_returns" : "matrix", dateColumn, returnColumns: numericColumns };
...
export function auditDataset(parsed, meta = {}) {
  ...
  const allScores = strategyMetrics.map((item) => item.sharpe).filter(Number.isFinite);
  ...
  const integrityChecks = integrityAudit(parsed, strategyMetrics);
  const score = scoreReport({ selected: selectedWithDeflation, pbo, validation, integrityChecks });
  const verdict = makeVerdict({ selected: selectedWithDeflation, pbo, validation, summary, score });
...
function emptyReport(summary, validation, meta) {
  return {
    ...
    validation,
    provenance: meta.provenance || null,
    selected: { name: "N/A", sharpe: null, psr: null, dsr: null, minTrackRecordLength: null, maxDrawdown: null, skewness: null, kurtosis: null },
    strategies: [],
    pbo: null,
    dataIntegrity: [],
...
function validate(parsed, meta) {
  ...
  if (allReturns.some((value) => value <= -1)) warnings.push("At least one return is less than or equal to -100%, which is not a valid simple return.");
...
function integrityAudit(parsed, strategyMetrics) {
  const checks = [];
  ...
  checks.push({ id: "matrix_correlation", severity: "warning", status: nearDuplicateColumns(parsed) ? "fail" : "pass", message: "Near-identical strategy columns weaken trial independence." });
  return checks;
}
...
function computeSeriesMetrics(returns) {
  ...
  return { observations: returns.length, mean: avg, volatility: sd, sharpe, skewness: skew, kurtosis: kurt, psr: psrValue, minTrackRecordLength: minTrl, maxDrawdown: maxDrawdown(returns), cumulativeReturn: cumulativeReturn(returns), hitRate: returns.filter((value) => value > 0).length / returns.length };
}
...
function computeDsr(selectedReturns, trialSharpes) {
  ...
  if (n <= 1 || selectedMetrics.sharpe == null) return { dsr: null, hurdle: null, effectiveTrials: n };
  ...
  return { dsr: psr(selectedMetrics.sharpe, selectedReturns.length, selectedMetrics.skewness, selectedMetrics.kurtosis, hurdle), hurdle, effectiveTrials: n };
}
...
function computePbo(parsed, blockCount = 8) {
  ...
  for (const trainBlocks of combos) {
    ...
    const sortedTest = [...test].sort((a, b) => a.score - b.score);
    const rank = sortedTest.findIndex((item) => item.name === selected.name) + 1;
    const w = rank / (sortedTest.length + 1);
  ...
  return { pbo: logitValues.filter((value) => value < 0).length / logitValues.length, combinations: logitValues.length, logitValues, inSampleRanks, outOfSampleRanks, probabilityOfLoss: losses / logitValues.length, degradation: mean(isPerf) - mean(oosPerf), selectedStrategyMedianOosRank: median(outOfSampleRanks) };
}

function scoreReport({ selected, pbo, validation, integrityChecks }) {
  const components = {
  ...
  if (pbo && pbo.pbo > 0.5) overrides.push("PBO above 0.5 caps decision at research_more.");
...
function makeVerdict({ selected, pbo, validation, summary, score }) {
  if (validation.errors.length || !selected || selected.psr == null) return { status: "Insufficient data", title: "Insufficient data", reason: "The dataset could not support a complete statistical audit." };
  ...
  if (pbo && pbo.pbo > 0.20) return { status: "Likely overfit", title: "Selection process looks overfit", reason: `PBO is ${formatPercent(pbo.pbo)}, meaning the selected in-sample winner often ranks poorly out of sample.` };
  if (score.overrides.length) return { status: "Research more", title: "Audit has unresolved blockers", reason: score.overrides.join(" ") };
  if (selected.psr >= 0.95 && (selected.dsr == null || selected.dsr >= 0.95) && enoughTrack) return { status: "Credible", title: "Backtest survives the main checks", reason: "The selected stream passes available statistical checks. Capital-readiness still requires cost, leakage, factor, and live-incubation review." };
...
function emptyScore(decision, overrides) {
  return { total: 0, components: { statisticalCredibility: 0, multipleTestingControl: 0, robustness: 0, costCapacity: 0, factorIndependence: 0, dataIntegrity: 0, liveIncubation: 0 }, decision, overrides };
}
...
function nearDuplicateColumns(parsed) { for (let i = 0; i < parsed.strategyNames.length; i += 1) for (let j = i + 1; j < parsed.strategyNames.length; j += 1) { const x = parsed.observations.map((row) => row.values[parsed.strategyNames[i]]); const y = parsed.observations.map((row) => row.values[parsed.strategyNames[j]]); if (Math.abs(correlation(x, y)) > 0.999) return true; } return false; }
